      ...
      <?php 
          ...
          if ( ! empty( $categories ) ) {
              foreach( $categories as $category ) {
                  $output .= '<a class="text-dark ml_5"  href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'codematra' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>,' . $separator;
              }
          }
              ...
              <div class="cminleft mr_10 my_5">
                <a href="<?php the_permalink($post->ID); ?>" class="d-inline-block tdn bg-light cimgwr w_50 h_50" >
                  ...
                  <?php if(has_post_thumbnail()) { ?>
                    <img class="h_50 img-thumbnail" src="<?php echo $image; ?>" alt="<?php the_title(); ?>" />
                  <?php } ?>
                </a>
              </div>
              <div class="cminright">
                ...
                <div class="cmeta mb_0 text-muted f12 d-flex align-items-center" title="Categories">
                  <i class="text-secondary fa fa-tag" aria-hidden="true"></i> <?php echo $catbadges; ?>
                </div>
              </div>
            </li>
        <?php    
