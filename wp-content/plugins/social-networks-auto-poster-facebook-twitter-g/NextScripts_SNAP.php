...
define( 'NextScripts_SNAP_Version' , '4.3.23' ); define( 'NextScripts_SNAP_Version_Date' , 'Oct 5, 2021' ); require_once "inc/nxs_functions_wp.php"; if(!defined( 'NXSSNAP_BASENAME' ) ) define( 'NXSSNAP_BASENAME', plugin_basename( __FILE__ ) );
...
if (true===nxs_doSystemInitCheck()) { //  error_reporting(E_ALL); ini_set('display_errors', '1');
    ...
    foreach (glob(NXS_PLPATH.'inc-cl/*.php') as $filename){ require_once $filename;  $filename = explode('/', $filename); $filename = array_pop($filename);
        //## V5
        if (stripos($filename,'.api')===false) {  $ntCode = strtoupper(str_ireplace('.php', '', $filename)); $ntLCode = strtolower($ntCode);
            $ntClassName = 'nxs_nt_' . $ntCode; if (class_exists($ntClassName)) { $nt = new $ntClassName; $subNts = [];
                if (!empty($nt->ntCons)) foreach ($nt->ntCons as $sNtL) { $sntClassName = 'nxs_ntConn_'.$ntCode.'_'.$sNtL; $sNt = new $sntClassName; $subNts[$sNtL] = $sNt->connInfo; }
                $nxs_avlblNTList['abc'][$ntLCode]['info'] = $nt->info;  $nxs_avlblNTList['abc'][$ntLCode]['subs'] = $subNts;
                $nxs_avlblNTList['byt'][$nt->info['type']][$ntLCode]['info'] = $nt->info; $nxs_avlblNTList['byt'][$nt->info['type']][$ntLCode]['subs'] = $subNts;
                $nxs_snapAvNts[] = $nt->info;
            }
        }
        //## /V5
    }
